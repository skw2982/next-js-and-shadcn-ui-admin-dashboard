...
type TabKey = "overview" | "stocks" | "realestate" | "savings" | "realized" | "target";
...
const TABS: { key: TabKey; label: string; num: string; icon: string }[] = [
  ...
  { key: "target", label: "목표가 시뮬", num: "6", icon: "🎯" },
];
...
export default function AssetMasterV2() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem("am_targets");
      if (saved) setTargetPrices(JSON.parse(saved) as Record<string, number>);
    } catch {}
    fetchAllData();
  ...
  const updateTargetPrice = useCallback((key: string, price: number) => {
    setTargetPrices((prev) => {
      const next = { ...prev, [key]: price };
      try { localStorage.setItem("am_targets", JSON.stringify(next)); } catch {}
      return next;
    });
  }, []);
  ...
  // ── Target Simulation ──
  const targetSimRows = useMemo(() => {
    return stocks.map((s) => {
      const key = `${s.account}__${s.name}`;
      const isOS = s.market.includes("해외");
      const rate = isOS ? exchangeRate : 1;
      const targetPrice = targetPrices[key] ?? s.current;
      const currentValKrw = s.current * rate * s.qty;
      const targetValKrw = targetPrice * rate * s.qty;
      const investedKrw = s.avg * rate * s.qty;
      const currentProfitKrw = currentValKrw - investedKrw;
      const targetProfitKrw = targetValKrw - investedKrw;
      const gainKrw = targetValKrw - currentValKrw;
      const targetYieldPct = investedKrw > 0 ? ((targetValKrw - investedKrw) / investedKrw) * 100 : 0;
      const targetChangePct = s.current > 0 ? ((targetPrice - s.current) / s.current) * 100 : 0;
      return { ...s, key, targetPrice, currentValKrw, targetValKrw, currentProfitKrw, targetProfitKrw, gainKrw, targetYieldPct, targetChangePct };
    });
  }, [stocks, targetPrices, exchangeRate]);

  const targetSummary = useMemo(() => {
    const currentTotal = targetSimRows.reduce((a, b) => a + b.currentValKrw, 0);
    const targetTotal = targetSimRows.reduce((a, b) => a + b.targetValKrw, 0);
    const currentProfit = targetSimRows.reduce((a, b) => a + b.currentProfitKrw, 0);
    const targetProfit = targetSimRows.reduce((a, b) => a + b.targetProfitKrw, 0);
    return { currentTotal, targetTotal, currentProfit, targetProfit, gain: targetTotal - currentTotal };
  }, [targetSimRows]);
    ...
    <div className="min-h-screen bg-[#0c0e12] text-slate-200 p-4 md:p-8 font-sans selection:bg-blue-500/30">
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        {activeTab === "target" && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gradient-to-br from-slate-900 to-slate-900/80 p-6 rounded-3xl border border-slate-800">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">현재 총 평가액</p>
                <p className="text-2xl font-black text-white">{fmtShort(targetSummary.currentTotal)}원</p>
                <p className={`text-xs mt-1 font-mono ${pctColor(targetSummary.currentProfit)}`}>
                  누적 {pctSign(targetSummary.currentProfit)}{fmtShort(targetSummary.currentProfit)}원
                </p>
              </div>
              <div className={`p-6 rounded-3xl border ${
                targetSummary.gain >= 0
                  ? "bg-gradient-to-br from-emerald-950/40 to-slate-900 border-emerald-900/50"
                  : "bg-gradient-to-br from-rose-950/40 to-slate-900 border-rose-900/50"
              }`}>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">🎯 목표 달성시 평가액</p>
                <p className={`text-2xl font-black ${targetSummary.gain >= 0 ? "text-emerald-300" : "text-rose-300"}`}>
                  {fmtShort(targetSummary.targetTotal)}원
                </p>
                <p className={`text-xs mt-1 font-mono ${pctColor(targetSummary.targetProfit)}`}>
                  누적 {pctSign(targetSummary.targetProfit)}{fmtShort(targetSummary.targetProfit)}원
                </p>
              </div>
              <div className={`p-6 rounded-3xl border ${
                targetSummary.gain >= 0
                  ? "bg-gradient-to-br from-blue-950/40 to-slate-900 border-blue-900/50"
                  : "bg-gradient-to-br from-rose-950/40 to-slate-900 border-rose-900/50"
              }`}>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">예상 추가 손익</p>
                <p className={`text-2xl font-black ${pctColor(targetSummary.gain)}`}>
                  {pctSign(targetSummary.gain)}{fmtShort(targetSummary.gain)}원
                </p>
                <p className="text-xs mt-1 text-slate-600 font-mono">
                  {targetSummary.currentTotal > 0
                    ? `${pctSign(targetSummary.gain)}${((targetSummary.gain / targetSummary.currentTotal) * 100).toFixed(2)}%`
                    : "—"}
                </p>
              </div>
            </div>
            ...
            <p className="text-[10px] text-slate-600 font-mono text-center">
              💡 목표가를 입력하면 자동 계산됩니다. 값은 브라우저에 저장됩니다.
            </p>

            {Object.entries(
              targetSimRows.reduce<Record<string, typeof targetSimRows>>((acc, row) => {
                if (!acc[row.account]) acc[row.account] = [];
                acc[row.account].push(row);
                return acc;
              }, {})
            ).map(([account, rows], idx) => {
              const acctCurrentTotal = rows.reduce((a, b) => a + b.currentValKrw, 0);
              const acctTargetTotal = rows.reduce((a, b) => a + b.targetValKrw, 0);
              const acctGain = acctTargetTotal - acctCurrentTotal;
              return (
                <Card key={account} className="overflow-hidden">
                  <div className="px-6 md:px-8 py-5 bg-slate-800/40 border-b border-slate-800 flex justify-between items-center flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      <div
                      ...
                      />
                      <span className="font-black text-white italic tracking-tight text-lg">🎯 {account}</span>
                    </div>
                    <div className="text-right">
                      <div className="text-[10px] text-slate-500 font-mono">현재 {fmtShort(acctCurrentTotal)} → 목표 {fmtShort(acctTargetTotal)}</div>
                      <div className={`text-sm font-black ${pctColor(acctGain)}`}>
                        {pctSign(acctGain)}{fmtShort(acctGain)}원
                        {acctCurrentTotal > 0 && (
                          <span className="ml-1 opacity-60 text-[10px]">
                            ({pctSign(acctGain)}{((acctGain / acctCurrentTotal) * 100).toFixed(2)}%)
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead>
                        <tr className="text-slate-500 text-[10px] font-black uppercase border-b border-slate-800/50">
                          ...
                          <th className="px-4 py-3 text-center">목표가 입력</th>
                          <th className="px-4 py-3 text-right hidden md:table-cell">등락</th>
                          <th className="px-4 py-3 text-right hidden md:table-cell">현재 수익</th>
                          <th className="px-4 py-3 text-right">목표 수익</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-800/60">
                        {rows.map((s) => {
                            ...
                            <tr key={s.key} className="hover:bg-white/[0.02] transition-colors">
                              <td className="px-6 py-4 font-bold text-slate-200">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    <div>{s.name}</div>
                                    <div className="text-[10px] text-slate-600 font-normal">{s.qty.toLocaleString()}주</div>
                                  </div>
                                </div>
                              </td>
                              <td className="px-4 py-4 text-right font-mono text-slate-300 text-xs">
                                {currency}{isOS ? s.current.toFixed(2) : fmt(s.current)}
                              </td>
                              <td className="px-4 py-4 text-center">
                                <input
                                  ...
                                  step={isOS ? "0.01" : "1"}
                                  min="0"
                                  value={s.targetPrice === s.current && targetPrices[s.key] === undefined ? "" : s.targetPrice}
                                  placeholder={isOS ? s.current.toFixed(2) : String(Math.round(s.current))}
                                  onChange={(e) => {
                                    const v = parseFloat(e.target.value);
                                    if (!isNaN(v) && v > 0) updateTargetPrice(s.key, v);
                                    else if (e.target.value === "") updateTargetPrice(s.key, s.current);
                                  }}
                                  className="w-28 bg-slate-950/80 text-blue-300 font-black text-xs text-right px-3 py-2 rounded-xl border border-slate-700 focus:border-blue-500 focus:outline-none transition-colors"
                                />
                              </td>
                              <td className={`px-4 py-4 text-right text-xs font-black hidden md:table-cell ${pctColor(s.targetChangePct)}`}>
                                {pctArrow(s.targetChangePct)} {Math.abs(s.targetChangePct).toFixed(2)}%
                              </td>
                              <td className={`px-4 py-4 text-right text-xs hidden md:table-cell ${pctColor(s.currentProfitKrw)}`}>
                                <div className="font-black">{pctSign(s.currentProfitKrw)}{fmtShort(s.currentProfitKrw)}원</div>
                                <div className="opacity-60 text-[10px] font-mono">
                                  {s.avg > 0 ? `${pctSign((s.current - s.avg) / s.avg * 100)}${(((s.current - s.avg) / s.avg) * 100).toFixed(1)}%` : "—"}
                                </div>
                              </td>
                              <td className={`px-4 py-4 text-right text-xs ${pctColor(s.targetProfitKrw)}`}>
                                <div className="font-black">{pctSign(s.targetProfitKrw)}{fmtShort(s.targetProfitKrw)}원</div>
                                <div className={`opacity-70 text-[10px] font-mono ${pctColor(s.targetYieldPct)}`}>
                                  {pctSign(s.targetYieldPct)}{s.targetYieldPct.toFixed(1)}%
                                </div>
                              </td>
                            </tr>
                          );
