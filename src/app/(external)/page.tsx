...
export default function AssetMasterFinalV2() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (sRows.length >= 2) {
        // 최신 엑셀 구조 반영: 구분(0), 계좌(1), 종목명(2), 평단(5), 현재가(6), 수량(7)
  ...
  const projectedNetWorth = totalStockVal + totalOtherAssets + projectedSavings - totalLiabilities;
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-amber-900/10 p-6 rounded-[30px] border border-amber-900/30">
                <div className="flex justify-between mb-2"><span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">해외 주식</span><span className={`font-black ${summ.os.y >= 0 ? 'text-rose-500' : 'text-blue-500'}`}>{summ.os.y.toFixed(2)}%</span></div>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {grouped[acc].items.map((s:any, i:number) => {
                        ...
                        const rate = s.market.includes('해외') ? exchangeRate : 1;
                        const prof = (s.current - s.avg) * rate * s.qty;
                        return (
                          <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                            ...
                            <td className={`px-8 py-5 text-right font-black ${isUp ? 'text-rose-500' : 'text-blue-500'}`}>
                              ...
                              <div className="text-[10px] opacity-60 font-medium">{isUp ? '+' : ''}{formatComma(prof)}원</div>
                            </td>
                          </tr>
                        );
                      })}
