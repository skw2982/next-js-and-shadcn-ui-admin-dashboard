...
export default function AssetMasterV2Final() {
  ...
  const getStockSummary = () => {
    ...
    stocks.forEach(s => {
      ...
      domVal += !isOS ? s.current * s.qty : 0;
      domInv += !isOS ? s.avg * s.qty : 0;
      osVal += isOS ? s.current * rate * s.qty : 0;
      osInv += isOS ? s.avg * rate * s.qty : 0;
    });
    return { 
      dom: { val: domVal, p: domVal - domInv, y: domInv > 0 ? ((domVal - domInv)/domInv*100) : 0 },
      os: { val: osVal, p: osVal - osInv, y: osInv > 0 ? ((osVal - osInv)/osInv*100) : 0 }
    };
  };
    ...
    <div className="min-h-screen bg-[#0c0e12] text-slate-200 p-4 md:p-8 font-sans">
      <div className="max-w-5xl mx-auto">
        <header className="mb-8 flex justify-between items-end border-b border-slate-800 pb-6">
          ...
          <div className="bg-amber-500/10 px-4 py-2 rounded-2xl border border-amber-500/20 text-amber-500 font-mono text-xs font-bold uppercase tracking-widest">Exchange: {exchangeRate.toFixed(2)}</div>
        </header>

        <nav className="flex gap-2 mb-10 overflow-x-auto pb-2 scrollbar-hide font-black">
          {['overview', 'stocks', 'realestate', 'savings', 'realized'].map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`px-6 py-3 rounded-2xl text-[10px] tracking-widest uppercase transition-all ${activeTab === t ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-500'}`}>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-gradient-to-br from-indigo-900/40 to-slate-900 p-12 rounded-[50px] border border-indigo-500/20 shadow-2xl">
              <p className="text-indigo-400 text-[10px] font-black uppercase tracking-widest mb-4 opacity-70">Current Net Worth</p>
              <h2 className="text-5xl md:text-7xl font-black text-white tracking-tighter">{formatComma(netWorth)}<span className="text-2xl font-light ml-2 opacity-30">KRW</span></h2>
            </div>
            <div className="bg-emerald-900/10 p-10 rounded-[50px] border border-emerald-900/30">
              <div className="flex justify-between items-center mb-8">
                <div><h3 className="text-xl font-black text-emerald-400 italic">CF Projection</h3></div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-slate-950/50 p-6 rounded-3xl text-center"><p className="text-[10px] text-slate-500 font-black uppercase mb-2 tracking-widest">예상 미래 순자산</p><p className="text-3xl font-black text-white">{formatComma(projectedNetWorth)}</p></div>
                <div className="bg-slate-950/50 p-6 rounded-3xl text-center"><p className="text-[10px] text-slate-500 font-black uppercase mb-2 tracking-widest">순자산 증감액</p><p className="text-3xl font-black text-emerald-400">+{formatComma(projectedNetWorth - netWorth)}</p></div>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'stocks' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-blue-900/10 p-6 rounded-3xl border border-blue-900/30">
                ...
                <p className="text-2xl font-black text-white">{formatComma(summ.dom.val)} 원</p>
              </div>
              <div className="bg-amber-900/10 p-6 rounded-3xl border border-amber-900/30">
                ...
                <p className="text-2xl font-black text-white">{formatComma(summ.os.val)} 원</p>
              </div>
            </div>
              ...
              <div key={acc} className="bg-slate-900 rounded-[30px] border border-slate-800 overflow-hidden shadow-xl">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="text-slate-500 text-[10px] font-black uppercase border-b border-slate-800 bg-slate-900/20">
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {grouped[acc].items.map((s:any, i:number) => {
                        ...
                        const profKrw = (s.current - s.avg) * rate * s.qty; // 여기서 환율 반영!
            ...
            {savings.map(s => {
              ...
              return (
                <div key={s.id} className="bg-[#0c0e12] p-6 rounded-3xl border border-slate-800 flex flex-col xl:flex-row gap-6 relative">
                  <button onClick={() => setSavings(savings.filter(sv => sv.id !== s.id))} className="absolute top-4 right-4 text-slate-600 hover:text-red-500">×</button>
